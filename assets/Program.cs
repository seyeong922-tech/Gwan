...
namespace EvolvingDesktopPet
{
    internal static class Program
    {
        ...
        private static void SelfTest()
        {
            ...
            string[] items = { "running_shoe", "pushup_bars", "research_paper", "linkedin", "chicken", "pork", "black_soda", "lettuce", "kt_wiz", "arsenal", "tft", "minecraft" };
            ...
            string[] premiumItems = { "running_shoe", "linkedin", "pork", "arsenal", "tft" };
            foreach (string item in premiumItems)
                if (PetForm.ItemStatGain(item) != 3) throw new Exception(item + " must award 3 stat points");
            string[] standardItems = { "pushup_bars", "research_paper", "chicken", "black_soda", "kt_wiz", "minecraft" };
            foreach (string item in standardItems)
                if (PetForm.ItemStatGain(item) != 1) throw new Exception(item + " must award 1 stat point");
    ...
    internal sealed class PetForm : Form
    {
        ...
        private LettuceMiniGameForm activeLettuceGame;
        ...
        public PetForm(string[] args)
        {
            ...
            if (Array.IndexOf(args, "--qa-spawn-item") >= 0)
            {
                ...
                spawnQaTimer.Tick += delegate { spawnQaTimer.Stop(); SpawnItem(true); };
                spawnQaTimer.Start();
            }
            ...
            if (Array.IndexOf(args, "--qa-black-soda") >= 0)
            {
                Timer sodaQaTimer = new Timer();
                sodaQaTimer.Interval = 350;
                sodaQaTimer.Tick += delegate
                {
                    sodaQaTimer.Stop();
                    AcceptItem("black_soda");
                };
                sodaQaTimer.Start();
            }

            int stage2QaIndex = Array.IndexOf(args, "--qa-set-stage2");
            if (stage2QaIndex >= 0 && stage2QaIndex + 1 < args.Length)
            {
                string qaStage2Path = args[stage2QaIndex + 1];
                if (Array.IndexOf(StatKeys, qaStage2Path) >= 0)
                {
                    stage = 2;
                    state["stage"] = 2;
                    state["stage2_path"] = qaStage2Path;
                    state["stage2_evolution_seen"] = true;
                    Dictionary<string, object> qaUnlocked = EnsureNestedState("unlocked_paths", EmptyUnlockedPaths());
                    qaUnlocked[qaStage2Path] = true;
                    SelectSprite();
                    SaveState();
                }
            }
            ...
            if (Array.IndexOf(args, "--qa-lettuce-success") >= 0)
            {
                Timer lettuceSuccessQaTimer = new Timer();
                lettuceSuccessQaTimer.Interval = 350;
                lettuceSuccessQaTimer.Tick += delegate
                {
                    lettuceSuccessQaTimer.Stop();
                    HandleLettuceMiniGameResult(true);
                };
                lettuceSuccessQaTimer.Start();
            }
        ...
        private static void MigrateState(Dictionary<string, object> result)
        {
            ...
            else if (!existingCounts.ContainsKey("black_soda"))
            {
                existingCounts["black_soda"] = ToInt(Get(existingCounts, "tomato", 0));
                result["item_counts"] = existingCounts;
            }
        ...
        private void BuildMenu()
        {
            ...
            menu.Items.Add("상호작용 아이템 바로 생성", null, delegate { SpawnItem(true); });
        ...
        private void SetDropFilter(string path)
        {
            ...
            {
                InteractionItemForm previousItem = activeItem;
                activeItem = null;
                previousItem.Close();
            }
            ScheduleNextItem();
        ...
        private void MotionTick(object sender, EventArgs e)
        {
            ...
            {
                if (now >= nextTalk && behavior != "talk" && behavior != "stomp") StartTalking();
                else if (now >= behaviorUntil)
                {
                    bool finishedStomp = behavior == "stomp";
                    speech = "";
                    recommendedTrack = null;
                    ChooseBehavior();
                    if (finishedStomp)
                ...
                else if (behavior == "stomp")
                {
                    double stompPhase = (behaviorUntil - now).TotalSeconds;
                    bob = -Math.Abs(Math.Sin(stompPhase * 10.5)) * 22;
                    pose = ((int)(stompPhase / 0.28) % 2 == 0) ? "angry" : "walk";
                }
            ...
            if (behavior != "stomp") CheckEvolution();
        }
        ...
        {
            if (activeItem == null && activeLettuceGame == null && DateTime.Now >= nextItem && !evolving) SpawnItem(false);
        }
        ...
        private void SpawnItem(bool replaceExisting)
        {
            if (activeLettuceGame != null && !activeLettuceGame.IsDisposed)
            {
                if (replaceExisting) ShowNote("상추 미니게임을 먼저 완료해줘!");
                return;
            }
            if (activeItem != null && !activeItem.IsDisposed)
            {
            ...
            kinds.Add("black_soda");
            kinds.Add("lettuce");
            List<string> candidates = new List<string>();
            foreach (string candidate in kinds)
                if (kinds.Count == 1 || candidate != lastSpawnedItemKind) candidates.Add(candidate);
            string kind = candidates[random.Next(candidates.Count)];
            lastSpawnedItemKind = kind;
            InteractionItemForm spawnedItem = new InteractionItemForm(kind, this, ToInt(Get(settings, "item_lifetime_seconds", 20)));
            activeItem = spawnedItem;
            ...
            spawnedItem.Location = new Point(x, y);
            spawnedItem.FormClosed += delegate
            {
                if (Object.ReferenceEquals(activeItem, spawnedItem))
                {
                    activeItem = null;
                    ScheduleNextItem();
                }
        ...
        internal void AcceptItem(string kind)
        {
            ...
            {
                SaveState();
                ShowExpression("contempt", "상추 함정! 전부 밟아버려!", 6.5);
                LettuceMiniGameForm game = new LettuceMiniGameForm(HandleLettuceMiniGameResult);
                activeLettuceGame = game;
                game.FormClosed += delegate
                {
                    if (Object.ReferenceEquals(activeLettuceGame, game))
                    {
                        activeLettuceGame = null;
                        ScheduleNextItem();
                    }
                };
                game.Show();
            ...
            if (kind == "black_soda")
            {
                string gained = StatKeys[random.Next(StatKeys.Length)];
                stats[gained] = ToInt(Get(stats, gained, 0)) + 1;
                state["last_stat"] = gained;
                ShowExpression("surprised", "정체불명의 검은 탄산! " + StatLabel(gained) + " +1", 3.0);
                SaveState();
                CheckEvolution();
                if (evolving) playMusicAfterEvolution = true;
                else SuggestMusic();
            ...
            string stat = (kind == "running_shoe" || kind == "pushup_bars") ? "exercise"
                ...
                : (kind == "chicken" || kind == "pork") ? "food"
                : (kind == "kt_wiz" || kind == "arsenal") ? "sports" : "gaming";
            int gain = ItemStatGain(kind);
            stats[stat] = ToInt(Get(stats, stat, 0)) + gain;
            state["last_stat"] = stat;
            string reaction = stat == "sports" ? "cheer" : stat == "study" ? "paper" : "happy";
            ShowExpression(reaction, StatLabel(stat) + " +" + gain, 2.8);
            SaveState();
            CheckEvolution();
            if (evolving) playMusicAfterEvolution = true;
            else SuggestMusic();
        ...
        internal static int ItemStatGain(string kind)
        {
            return kind == "running_shoe" || kind == "linkedin" || kind == "pork" || kind == "arsenal" || kind == "tft" ? 3 : 1;
        }

        private void HandleLettuceMiniGameResult(bool success)
        {
            Dictionary<string, object> stats = EnsureNestedState("stats", EmptyStats());
            if (success)
            {
                string gained = StatKeys[random.Next(StatKeys.Length)];
                stats[gained] = ToInt(Get(stats, gained, 0)) + 5;
                state["last_stat"] = gained;
                behavior = "stomp";
                behaviorUntil = DateTime.Now.AddSeconds(3.2);
                speech = "";
                recommendedTrack = null;
                SetPose("angry");
                ShowNote("상추 짓밟기 성공! " + StatLabel(gained) + " +5");
                SaveState();
                return;
            }
        ...
        private static Dictionary<string, object> EmptyItemCounts()
        {
            return Dict("running_shoe", 0, "pushup_bars", 0, "research_paper", 0, "linkedin", 0,
                "chicken", 0, "pork", 0, "black_soda", 0, "lettuce", 0,
        ...
        internal static string ItemLabel(string kind)
        {
            ...
            if (kind == "black_soda") return "정체불명의 검은 탄산";
        ...
        private void CheckEvolution()
        {
            ...
            if (stage == 2 && !String.IsNullOrEmpty(newlyUnlocked))
            {
                string currentPath = Convert.ToString(Get(state, "stage2_path", ""));
                if (newlyUnlocked != currentPath)
                {
                    StartEvolution(2, newlyUnlocked);
                    return;
                }
            }
        ...
        private void StartEvolution(int target, string path)
        {
            ...
            bool changingStage2Path = target == 2 && stage == 2
                && Array.IndexOf(StatKeys, path) >= 0
                && path != Convert.ToString(Get(state, "stage2_path", ""));
            if (target == stage && !changingStage2Path) return;
            currentPose = "idle";
            SelectSprite();
            evolving = true;
    ...
    internal sealed class LettuceMiniGameForm : Form
    {
        private readonly Timer countdownTimer = new Timer();
        private readonly Random random = new Random();
        private readonly Action<bool> completed;
        private readonly Button stompButton = new Button();
        private readonly Label titleLabel = new Label();
        private readonly Label statusLabel = new Label();
        private readonly DateTime deadline = DateTime.Now.AddSeconds(6.5);
        private int remainingTargets = 8;
        private bool resultDelivered;

        public LettuceMiniGameForm(Action<bool> completed)
        {
            ...
            titleLabel.Text = "시간 안에 나타나는 버튼을 전부 밟아라!";
            titleLabel.ForeColor = Color.FromArgb(135, 206, 235);
            titleLabel.Font = new Font("Malgun Gothic", 14, FontStyle.Bold);
            titleLabel.TextAlign = ContentAlignment.MiddleCenter;
            titleLabel.SetBounds(15, 12, 490, 38);
            Controls.Add(titleLabel);

            statusLabel.ForeColor = Color.White;
            statusLabel.Font = new Font("Malgun Gothic", 10.5f, FontStyle.Bold);
            statusLabel.TextAlign = ContentAlignment.MiddleCenter;
            statusLabel.SetBounds(15, 286, 490, 30);
            Controls.Add(statusLabel);

            stompButton.Text = "밟기!";
            stompButton.Font = new Font("Malgun Gothic", 12, FontStyle.Bold);
            stompButton.FlatStyle = FlatStyle.Flat;
            stompButton.FlatAppearance.BorderSize = 3;
            stompButton.FlatAppearance.BorderColor = Color.FromArgb(135, 206, 235);
            stompButton.BackColor = Color.FromArgb(76, 175, 80);
            stompButton.ForeColor = Color.White;
            stompButton.Size = new Size(92, 58);
            stompButton.Click += OnStompClicked;
            Controls.Add(stompButton);

            countdownTimer.Interval = 80;
            countdownTimer.Tick += OnCountdown;
            Shown += delegate { MoveTarget(); UpdateStatus(); countdownTimer.Start(); };
        }

        private void OnStompClicked(object sender, EventArgs e)
        {
            remainingTargets--;
        ...
        {
            int left = random.Next(18, Math.Max(19, ClientSize.Width - stompButton.Width - 18));
            int top = random.Next(60, Math.Max(61, 270 - stompButton.Height));
            stompButton.Location = new Point(left, top);
        }

        private void OnCountdown(object sender, EventArgs e)
        {
            if (DateTime.Now >= deadline) DeliverResult(false);
            else UpdateStatus();
        }

        private void UpdateStatus()
        {
            double seconds = Math.Max(0, (deadline - DateTime.Now).TotalSeconds);
            statusLabel.Text = "남은 버튼 " + remainingTargets + "개   |   남은 시간 " + seconds.ToString("0.0") + "초";
        }

        private void DeliverResult(bool success)
        {
        ...
        {
            if (disposing) countdownTimer.Dispose();
